        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <div className="flex gap-4 flex-wrap">
              <Link href="/contact" className="bg-primary text-white px-8 py-4 rounded-lg font-bold hover:shadow-lg transition-all">{t('ctaPrimary')}</Link>
              <Link href="/portfolio" className="border border-gray-300 text-gray-800 px-8 py-4 rounded-lg font-bold hover:bg-gray-50 transition-all">{t('ctaSecondary')}</Link>
            </div>
          </div>
        ...
        <div className="px-6 md:px-12 max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
          {(['years','projects','industries','retention'] as const).map((key) => (
            <div key={key}>
              ...
              <p className="text-sm text-gray-500 uppercase tracking-wider mt-2">{t(`stats.${key}Label`)}</p>
            </div>
          ))}
        </div>
      </section>
      <section className="py-24 px-6 md:px-12 max-w-7xl mx-auto">
        <div className="bg-gray-50 rounded-3xl overflow-hidden grid grid-cols-1 lg:grid-cols-2 border border-gray-200">
          <div className="bg-gray-200/50 flex items-center justify-center p-12">
            <div className="w-full h-[400px] bg-white/50 backdrop-blur-sm rounded-xl border border-white flex items-center justify-center text-gray-500 italic">{t('caseStudy.screenshot')}</div>
          </div>
          <div className="p-16 flex flex-col justify-center">
            <span className="text-primary font-bold tracking-widest text-sm mb-4 uppercase">{t('caseStudy.label')}</span>
            <h2 className="text-3xl md:text-4xl font-bold mb-6">{t('caseStudy.title')}</h2>
            <p className="text-base text-gray-600 mb-8 leading-relaxed">{t('caseStudy.description')}</p>
            <ul className="space-y-4 mb-10">
              {(['feature1','feature2','feature3'] as const).map((f) => (
                <li key={f} className="flex items-center gap-3"><CheckCircle className="w-5 h-5 text-primary" /><span>{t(`caseStudy.${f}`)}</span></li>
              ))}
            </ul>
            <a href="https://qianlima.co.id" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-primary font-bold group">
              {t('caseStudy.cta')} <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
            </a>
          </div>
        </div>
      </section>
